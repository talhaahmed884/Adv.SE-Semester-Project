...
public class ToDoListNameValidator extends Validator<String> {
    ...
    protected void performValidation(String name, ValidationResultBuilder resultBuilder) {
        if (name == null) {
            resultBuilder.addError("Todo list name cannot be null");
            return;
        }
        ...
        if (trimmed.isEmpty()) {
            resultBuilder.addError("Todo list name cannot be empty");
            return;
        }

        if (trimmed.length() < MIN_LENGTH) {
            resultBuilder.addError("Todo list name must be at least " + MIN_LENGTH + " character");
        }

        if (trimmed.length() > MAX_LENGTH) {
            resultBuilder.addError("Todo list name must not exceed " + MAX_LENGTH + " characters");
        }
    }
}
